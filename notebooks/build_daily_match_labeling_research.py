...
- **Yes, partially, if days are processed in order and the system keeps a durable model-line-wide identifier-to-VID map.** The current day's or campaign-to-date match signal can determine how *newly seen* identifiers are placed into shared pools. Old assignments never move.
...
- **The most promising direction is a sequential, memoized overlap allocator driven by a provider-supplied cumulative calibration rule.** It should preserve each EDP's reach exactly and solve shared-pool quotas jointly. The experiments here establish feasibility and failure modes; they do not yet validate a production model.
...
"""
    ...
    markdown(
        r"""
## 10. Recommended research architecture
...
3. Have the model provider supply and version the function that converts campaign context, scale, email availability, and overlap summaries into target pairwise and selected higher-order overlaps.
4. Hold each EDP's pool size fixed and adjust only overlap among the EDP pools. Preserve direct shared-email anchors, then jointly allocate only newly seen proprietary fallback identifiers into private, pair, and structured higher-order shared slots. Do not let a smaller hash pool manufacture overlap through same-EDP collisions.
...
The current simulation does **not** establish that the tested 64-lane routing heuristic is good enough. It establishes that the architecture is feasible and that a richer constrained quota solver is needed. The five-EDP oracle Venn allocator is evidence that such a solver can encode cumulative subset geometry; its remaining arbitrary-window error defines the next research problem.
"""
    ),
    markdown(
        r"""
## 11. Other designs worth considering
...
"""
    ...
    markdown(
        r"""
## 12. What to validate with real data
...
"""
    ...
    markdown(
        r"""
## 13. What a 5,000-person panel can and cannot establish
...
"""
    ...
    markdown(
        r"""
## 14. Research and implementation references
